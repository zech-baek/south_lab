...
if "32bit" in platform.architecture():
    dll = f"{root_dir}/misc/dll/ch341_x32.dll"
else:
    dll = f"{root_dir}/misc/dll/ch341_x64.dll"


def log_wrapping(header, message, is_logging=True):
    
    if is_logging:
        log.infoLog(f"[{header} {sys._getframe(2).f_code.co_name}] {message}")
...
class ch341(object):
    ...
    def i2c_read(self, i2c_address, reg):
        ...
        if self.CH341ReadI2C(self.index, i2c_address, reg, ctypes.byref(self.value)) == True:
            ...
            log_wrapping(f"ch341", f"i2c read from {color.green}address {i2c_address:#04x} {color.yellow}register {reg:#04x} {color.cyan}return {ret:#04x}{color.end}", self.logging)
            return ret
        else:
            '''
            ...
            '''
            log_wrapping(f"ch341", f"{color.red}read transaction failed{color.end}", self.logging)
            pass
    ...
    def i2c_write(self, i2c_address, reg, w_val):
        ...
        log_wrapping(f"ch341", f"i2c write to {color.green}address {i2c_address:#04x} {color.yellow}register {reg:#04x} {color.cyan}value {w_val:#04x}{color.end}", self.logging)
    ...
    def smbus_scan(self):
        ...
        for i in range(0x2, 0x70):
            ...
            log_wrapping(f"ch341", f"temp_store : {i}={temp_store}", self.logging)
    ...
    def _write(self, i2c_address, addr, data):
        ...
        if self.dll.CH341StreamI2C(self.index, 3, obuf, 0, ibuf):
            log_wrapping(f"ch341", f"i2c write to address {i2c_address:#04x}, register {obuf}, value {ibuf}", self.logging)
            pass
        else:
            log_wrapping(f"ch341", f"[ch341 {sys._getframe(2).f_code.co_name}] write transaction failed", self.logging)
    ...
    def _read(self, i2c_address, addr):
        ...
        if self.dll.CH341StreamI2C(self.index, 2, obuf, 1, ibuf):
            log_wrapping(f"ch341", f"i2c read from address {i2c_address:#04x}, register {obuf} value {ibuf}", self.logging)
            return ibuf[0] & 0xff
        else:
            log_wrapping(f"ch341", f"[ch341 {sys._getframe(2).f_code.co_name}] read transaction failed", self.logging)
    ...
    def i2c_speed(self, value):
        ...
        if self.dll.CH341SetStream(self.index, value):
            log_wrapping(f"ch341", f"[ch341 {sys._getframe(2).f_code.co_name}] open the device with default {color.blue}{scl_speed[value]} clock speed{color.end}", self.logging)
        else:
            log_wrapping(f"ch341", f"[ch341 {sys._getframe(2).f_code.co_name}] {color.red}failed to set the i2C speed{color.end}", self.logging)
    ...
    def i2c_read_bulk(self, slave, addr, length):
        ...
        if self.dll.CH341StreamI2C(self.index, 2, obuf, length, ibuf):
            ...
            log_wrapping(f"ch341", f"[ch341 {sys._getframe(2).f_code.co_name}] {color.red}failed to open ch341{color.end}", self.logging)
            return 0